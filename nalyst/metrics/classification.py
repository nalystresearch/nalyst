...
def _prepare_binary_rank_data(
    y_true: np.ndarray,
    y_score: np.ndarray,
    *,
    pos_label: int = 1,
) -> tuple[np.ndarray, np.ndarray]:
    """Return binary labels and scores sorted by score descending."""
    ...
    if y_score.ndim != 1:
        raise ValueError("y_score must be a 1D array for binary metrics")

    binary_true = (y_true == pos_label).astype(int)
    order = np.argsort(-y_score, kind="mergesort")

    return binary_true[order], y_score[order]


def auc(x: np.ndarray, y: np.ndarray) -> float:
    """Compute the area under a curve described by points (x, y)."""

    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)

    if x.shape != y.shape:
        raise ValueError("x and y must have the same shape to compute auc")
...
def roc_curve(
    ...
    *,
    ...
    binary_true, sorted_scores = _prepare_binary_rank_data(
        y_true, y_score, pos_label=pos_label
    )

    tp = np.cumsum(binary_true, dtype=float)
    fp = np.cumsum(1 - binary_true, dtype=float)

    pos_total = max(tp[-1], 1.0)
    neg_total = max(fp[-1], 1.0)

    tpr = np.r_[0.0, tp / pos_total]
    fpr = np.r_[0.0, fp / neg_total]
    thresholds = np.r_[np.inf, sorted_scores]
...
def roc_auc_score(
    ...
    *,
    ...
    multi_class : {"raise", "ovr", "ovo"}, default="raise"
        Multiclass strategy.
    pos_label : int, default=1
        Label to treat as the positive class for binary problems.
    ...
    if y_score.ndim == 1:
        ...
        for i, cls in enumerate(classes):
            ...
            fpr, tpr, _ = roc_curve(binary_true, cls_score, pos_label=1)
            auc_val = auc(fpr, tpr)
            aucs.append(auc_val)
